...
class App():
    ...
    def taobutton(self):
        ...
        self.luudiem1 = Button(self.root, text= "Choose Folder", width= "15", height="2", font= "16", command = self.chamnhieu).place(x=20, y=440)
    ...
    def addlist(self):
        self.dssv = tkFileDialog.askopenfilename(initialdir="/home/hieu/Documents/Test", title="Add list",filetypes=(("file", ".txt"), ("all file", "*.*")))
        self.list = []
        self.coutar = 0
        f = open(self.dssv, "r")
        for i in f:
    ...
    def openfile(self):
        self.file1 = []
        self.filee = tkFileDialog.askopenfilename(initialdir="/home/hieu/Documents/Test", title="Choose Test", filetypes=(("pictuue",".jpg"),("all file","*.*")))
    ...
    def nhapdapan(self):
        self.dapan = tkFileDialog.askopenfilename(initialdir="/home/hieu/Documents/Test", title="Import answers", filetypes=(("file",".txt"),("all file","*.*")))
    ...
    def luudiem(self,tenn, diemm):
        f = open("/home/hieu/Documents/opencv/Ketqua/result.txt", "w")
        f.write(str(tenn)+"         "+str(diemm)+"          "+str(self.date.strftime("%Y%m%d %H:%M:%S")+"\n"))
        f.close()

    def luubangdiem(self):
        duongdan = "/home/hieu/Documents/"+str(self.date.strftime("%Y%m%d %H:%M:%S"))
        with open("/home/hieu/Documents/opencv/Ketqua/result.txt","r") as rf:
            with open(duongdan,"w+") as wf:
                wf.write("MSSV            Score        Graded at\n")
                for line in rf:
                    wf.write(line)
        f = open("/home/hieu/Documents/result.txt", "w")
        f.write("")
        f.close()
    ...
    def luubaikt(self,fileluuu, tenluuu):
        ...
        cv2.imwrite("/home/hieu/Documents/{0}.jpg".format(self.tenbailuu), self.baiin)
    ...
    def guimaildi(self):
        ...
        f = open("/home/hieu/Documents/result.txt", "r")
        for i in f:
        ...
        for i in range(len(self.dsmail)):
            for j in range(len(self.list)):
                if(self.dsmail[i][0] == self.list[j][0]):
                    ...
                    duongdan = "/home/hieu/Documents/"+self.list[j][0]+"-"+self.dsmail[j][2] +"-"+self.dsmail[j][3] + ".jpg"
...
#show hinh cat
            ...
            f = open("/home/hieu/Documents/result.txt", "a")
            f.write(
